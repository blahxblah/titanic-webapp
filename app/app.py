...
r = requests.post(url="https://titanic-ml-api-kjh.fly.dev/preprocess/", data=dataJSON)
preprocessedData = r.json()
preprocessedJSON = json.dumps(preprocessedData)

# make prediction by making post request to the API
pred = requests.post(url="https://titanic-ml-api-kjh.fly.dev/predict/", data=preprocessedJSON)
